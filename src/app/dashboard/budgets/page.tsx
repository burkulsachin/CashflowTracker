...
export default function BudgetsPage() {
  ...
  const spendingByCategory = useMemo(() => {
    ...
    transactions.forEach(tx => {
      ...
      if (tx.type === 'expense' && isWithinInterval(txDate, { start: monthStart, end: monthEnd })) {
        spending[tx.categoryId] = (spending[tx.categoryId] || 0) + tx.amountMinor;
      }
    });
  ...
  const totalSpent = useMemo(() => {
      return Object.values(spendingByCategory).reduce((acc, amount) => acc + amount, 0);
  }, [spendingByCategory]);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex-1">
          ...
          <p className="text-muted-foreground">
            Manage your spending for {format(now, 'MMMM yyyy')}.
          </p>
        </div>
      ...
      {monthlyBudgets.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-muted bg-card p-12 text-center">
          ...
          <h3 className="text-xl font-semibold">No budgets set for this month</h3>
